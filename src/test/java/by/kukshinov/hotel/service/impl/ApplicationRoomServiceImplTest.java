...
public class ApplicationRoomServiceImplTest {
    ...
    private static final Application APPROVED_APPLICATION = new Application(ID, new Byte("4"), ApartmentType.BUSINESS, LocalDate.now(), LocalDate.now(), ApplicationStatus.APPROVED, new BigDecimal("505"), ROOM_ID, 2L);
    ...
    public void testFindApplicationRoomShouldReturnApprovedApplicationWithRoom() throws DaoException, ServiceException {
        ...
        when(applicationDao.findApprovedAppById(anyLong())).thenReturn(Optional.of(APPROVED_APPLICATION));
        ...
        ApplicationRoom actual = applicationUsernameService.findApplicationRoom(ID);
    ...
    public void testFindApplicationRoomShouldThrowServiceExceptionWhenApplicationIsNotFound() throws DaoException, ServiceException {
        ApplicationRoomService applicationUsernameService = new ApplicationRoomServiceImpl(helperFactory);
        when(applicationDao.findApprovedAppById(anyLong())).thenReturn(Optional.empty());
        when(roomDao.findById(ROOM_ID)).thenReturn(Optional.of(ROOM));

        applicationUsernameService.findApplicationRoom(ID);
    }

    @Test(expectedExceptions = ServiceException.class)
    public void testFindApplicationRoomShouldThrowServiceExceptionWhenRoomIdIsNull() throws DaoException, ServiceException {
        ApplicationRoomService applicationUsernameService = new ApplicationRoomServiceImpl(helperFactory);
        when(applicationDao.findApprovedAppById(anyLong())).thenReturn(Optional.of(DENIED_APPLICATION));
        when(roomDao.findById(ROOM_ID)).thenReturn(Optional.of(ROOM));
